...
variable "aws_iam_roles" {
  ...
  description = "AWS roles to create. If you set the value `cognito` in `assumable_by_federated`, it will be replaced by the newly created Cognito instance, eg. `{ MyRole = { assumable_by_federated = [\"cognito\"] } }`"
  default     = {}
}

variable "aws_iam_groups" {
  type        = map(map(list(string)))
  description = "AWS groups to create. It should be specified using a map of groups and their attributes, eg. `{ MyGroup = { policies = [\"arn:xxx\", ...], assume_roles = [\"arn:xxx\", ...] }, ...}`"
  default     = {}
}

variable "aws_iam_users" {
  type        = any
  description = "AWS users to create. You can specify a simple list, eg. `[\"user-1\", ...]` or a map of users and their groups, eg. `{ user-1 = [\"MyGroup\", ...], ...}`."
  default     = {}
}
...
variable "aws_cognito_user_fields" {
  description = "User profile fields to add to your Cognito user pool, eg. email, birthdate, twitter"
  default     = {}
}
